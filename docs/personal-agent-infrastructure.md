...
*Last updated 2026-04-25 (PR 6 landed: `SavedConfig.budget?: { perSessionCapUsd, warnThresholds? }` top-level user-config path; pre-call budget check at agent-loop iteration head; `budget_block` and `budget_warning` audit actions; CLI banner surfaces cost + remaining budget when capped; **honest scope:** post-spend enforcement only — pre-call cost estimation deferred; §6 note explains the PR 6 scope; §10 PR 6 marked done; §12 measurement signal updated to "visible on every CLI session"). Earlier passes: 2026-04-25 PR 5 (router rename + agent wire-up, same-provider only); 2026-04-25 PR 4 (capability-driven gating + move-money rejection); 2026-04-25 PR 3 (label population + CI enforcement); 2026-04-25 PR 2 (CapabilityLabel union + slot); 2026-04-25 split `move-money` from `spend-money`; 2026-04-24 engineering-contract discipline; 2026-04-24 user-owned multi-device-over-time vision. Against `main @ 6b045ed` + this PR.*
