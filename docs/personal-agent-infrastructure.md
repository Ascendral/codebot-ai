...
*Last updated 2026-04-25 (PR 8 landed: Gmail migrated to the §8 contract — 5 existing actions, per-verb capability labels, preview + redact + idempotency-unsupported declarations on send/draft, 401/403 reauth classifier throwing `ConnectorReauthError`, no new actions added; compliance 0/50 → 5/50 with gmail at 5/5; existing Gmail behavior preserved). Earlier passes: 2026-04-25 PR 7 refined (PR #31: idempotency union); PR 7 landed (PR #30: connector contract foundation); PR 6 (per-session budget cap + threshold audits); PR 5 (router rename + wire-up, same-provider only); PR 4 (capability-driven gating + move-money rejection); PR 3 (label population + CI enforcement); PR 2 (CapabilityLabel union + slot); split `move-money` from `spend-money`; 2026-04-24 engineering-contract discipline; user-owned multi-device-over-time vision. Against `main @ fb2fbda` + this PR.*
