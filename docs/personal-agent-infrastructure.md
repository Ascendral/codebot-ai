...
11. **PR 10+ — remaining Phase 1 connectors**, one per PR (Slack next; then Google Calendar, Notion, etc.). Each must pass `assertContractClean` cleanly.
...
| **Connector contract compliance** | **WIRED** | `src/connectors/contract-compliance.test.ts` (2 tests). Runs on every CI matrix entry. Current per-connector readout: `github 7/7 (100%)`, `gmail 5/5 (100%)`, every other registered connector `0/X (0%)`. Aggregate: **12/47 (26%)** across 10 registered connectors (denominator dropped from 50 to 47 on 2026-04-26 when `openai-images` was unregistered + deleted). New connector PRs must call `assertContractClean(new MyConnector())` and pass with zero violations (hard-fail). Existing connectors report-only. |
...
*Last updated 2026-04-26 (`openai-images` connector removed: source + test files deleted, exports stripped, registration removed; compliance denominator 50 → 47, ratio 12/50 → 12/47 (26%). Reasoning: paid OpenAI image generation was dead weight in the registry, hadn't been deliberately designed against `spend-money`, and dragged the compliance metric. The numerator is unchanged. `spend-money` capability label stays test-only-exercised pending a deliberate paid-API design pass.). Earlier: PR 9 GitHub migration; §12 honesty pass; honesty pass PR #33; PR 8 Gmail migration; PR 7 + refinement (connector contract); PR 6 budget; PR 5 router; PR 4 capability gating; PR 3 label population; PR 2 capability slot; pre-sprint security work. Against `main @ 8e177cd` + this PR.*
