 ...
 * `AGENTTRAIL_LOG_LEVEL`. Default is `warn` (current behavior preserved).
 * `AGENTTRAIL_LOG_LEVEL=error` quiets init warnings. `silent` kills
...
function readLevel(): LogLevel {
  const raw = (process.env.AGENTTRAIL_LOG_LEVEL || '').toLowerCase();
...
/** Re-read AGENTTRAIL_LOG_LEVEL (useful if env changes mid-process). */
...
export const log = {
  debug(...args: unknown[]): void {
    if (shouldLog('debug')) console.error('[AgentTrail:debug]', ...args);
  },
  info(...args: unknown[]): void {
    if (shouldLog('info')) console.error('[AgentTrail]', ...args);
  },
