/**
 * Policy Engine for AgentTrail v2.1.5
 *
 * Loads, validates, and enforces declarative security policies.
 * Policy files: .agenttrail/policy.json (project) + ~/.agenttrail/policy.json (global)
...
export function loadPolicy(projectRoot?: string): Policy {
  ...
  // Load global policy
  const globalPath = trailPath('policy.json');
  const globalPolicy = loadPolicyFile(globalPath);

  // Load project policy
  const projectPath = path.join(root, '.agenttrail', 'policy.json');
...
export class PolicyEnforcer {
  ...
  getBranchPrefix(): string {
    return this.getEffectivePolicy().git?.branch_prefix || 'agenttrail/';
  }
...
/**
 * Generate a default policy file content for `agenttrail --init-policy`.
 */
