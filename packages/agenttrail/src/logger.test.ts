 ...
 *   - AGENTTRAIL_LOG_LEVEL env var controls it
...
describe('logger', () => {
  ...
  beforeEach(() => {
    origEnv = process.env.AGENTTRAIL_LOG_LEVEL;
    calls = [];
  ...
  afterEach(() => {
    ...
    if (origEnv === undefined) delete process.env.AGENTTRAIL_LOG_LEVEL;
    else process.env.AGENTTRAIL_LOG_LEVEL = origEnv;
    refreshLogLevel();
  });
  ...
  it('refreshLogLevel re-reads AGENTTRAIL_LOG_LEVEL', () => {
    process.env.AGENTTRAIL_LOG_LEVEL = 'silent';
    refreshLogLevel();
    assert.strictEqual(getLogLevel(), 'silent');

    process.env.AGENTTRAIL_LOG_LEVEL = 'debug';
    refreshLogLevel();
    assert.strictEqual(getLogLevel(), 'debug');
  });

  it('invalid AGENTTRAIL_LOG_LEVEL falls back to default warn', () => {
    process.env.AGENTTRAIL_LOG_LEVEL = 'bogus-value';
    refreshLogLevel();
    assert.strictEqual(getLogLevel(), 'warn');
  });

  it('info prefixes messages with [AgentTrail]', () => {
    setLogLevel('info');
    log.info('hello');
    const lastCall = calls[calls.length - 1];
    assert.strictEqual(lastCall.args[0], '[AgentTrail]');
    assert.strictEqual(lastCall.args[1], 'hello');
  });

  it('debug prefixes messages with [AgentTrail:debug]', () => {
    setLogLevel('debug');
    log.debug('trace');
    const lastCall = calls[calls.length - 1];
    assert.strictEqual(lastCall.args[0], '[AgentTrail:debug]');
