/**
 * Encryption at rest for AgentTrail v2.1.0
 *
 ...
 *
 * Encryption is opt-in: set AGENTTRAIL_ENCRYPTION_KEY env var or
...
const HEADER = 'CBE1';       // AgentTrail Encrypted v1 — 4-byte magic header

export interface EncryptionConfig {
  /** Enable encryption at rest */
  enabled: boolean;
  /** Passphrase for key derivation (or use AGENTTRAIL_ENCRYPTION_KEY env var) */
  passphrase?: string;
}
...
export function getPassphrase(config?: EncryptionConfig): string | null {
  ...
  const envKey = process.env.AGENTTRAIL_ENCRYPTION_KEY;
