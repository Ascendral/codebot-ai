 ...
 * Can be suppressed with AGENTTRAIL_QUIET=1 for CI/test environments.
 */
...
export function warnNonFatal(context: string, error: unknown): void {
  if (process.env.AGENTTRAIL_QUIET === '1') return;
  ...
  process.stderr.write(`[agenttrail:warn] ${context}: ${msg}\n`);
}
