...
describe('Encryption — getPassphrase', () => {
  const originalEnv = process.env.AGENTTRAIL_ENCRYPTION_KEY;

  afterEach(() => {
    if (originalEnv !== undefined) {
      process.env.AGENTTRAIL_ENCRYPTION_KEY = originalEnv;
    } else {
      delete process.env.AGENTTRAIL_ENCRYPTION_KEY;
    }
  });
  ...
  it('returns env var when set', () => {
    process.env.AGENTTRAIL_ENCRYPTION_KEY = 'env-key-123';
  ...
  it('env var takes priority over config', () => {
    process.env.AGENTTRAIL_ENCRYPTION_KEY = 'env-wins';
...
describe('Encryption — isEncryptionEnabled', () => {
  ...
  it('returns true when env var is set', () => {
    process.env.AGENTTRAIL_ENCRYPTION_KEY = 'my-key';
...
describe('Encryption — encryptLine/decryptLine', () => {
  ...
  beforeEach(() => {
    process.env.AGENTTRAIL_ENCRYPTION_KEY = 'line-encryption-key';
  });
...
describe('Encryption — encryptContent/decryptContent', () => {
  ...
  beforeEach(() => {
    process.env.AGENTTRAIL_ENCRYPTION_KEY = 'content-encryption-key';
  });
...
describe('Encryption — round-trip through storage layers (v2.1.5)', () => {
  ...
  beforeEach(() => {
    process.env.AGENTTRAIL_ENCRYPTION_KEY = 'v2.1.5-round-trip-key';
  });
