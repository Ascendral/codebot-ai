...
describe('loadPolicy', () => {
  const testDir = path.join(os.homedir(), '.agenttrail', `test-policy-${Date.now()}`);

  beforeEach(() => {
    fs.mkdirSync(path.join(testDir, '.agenttrail'), { recursive: true });
  });
  ...
  it('loads project policy file', () => {
    ...
    fs.writeFileSync(path.join(testDir, '.agenttrail', 'policy.json'), policyContent);
    const policy = loadPolicy(testDir);
...
describe('PolicyEnforcer — RBAC applies to all methods (v2.1.5)', () => {
  const fullRbacPolicy = {
    ...
    git: { always_branch: true, branch_prefix: 'agenttrail/', never_push_main: true, require_tests_before_commit: false },
