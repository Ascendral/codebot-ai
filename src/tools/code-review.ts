...
const SECURITY_PATTERNS: Array<{ pattern: RegExp; rule: string; message: string; severity: Issue['severity'] }> = [
  ...
  {
    pattern: /child_process.*exec(?!Sync)/,
    rule: 'unsafe-exec',
  ...
  },
  {
    pattern: /innerHTML\s*=/,
    rule: 'no-innerhtml',
    message: 'innerHTML is vulnerable to XSS attacks',
    severity: 'warning',
  },
  {
    pattern: /document\.write\s*\(/,
    rule: 'no-document-write',
    message: 'document.write() is a security and performance issue',
    severity: 'warning',
  },
  {
  ...
  {
    pattern: /\bsqlite3?\s.*\+\s*(?:req\.|args\.|input)/i,
    rule: 'sql-injection',
  ...
  },
  {
    pattern: /https?:\/\/[^'"]*['"]\s*\+/,
    rule: 'url-injection',
    message: 'String concatenation in URL — possible injection',
    severity: 'warning',
  },
  {
...
export class CodeReviewTool implements Tool {
  name = 'code_review';
  description =
    'Review code for security issues, complexity, and code smells. Actions: security, complexity, review (full).';
  ...
  parameters = {
    ...
    properties: {
      action: {
        type: 'string',
        description:
          'Action: security (scan for vulnerabilities), complexity (function/nesting analysis), review (full review)',
      },
  ...
  async execute(args: Record<string, unknown>): Promise<string> {
    ...
    switch (action) {
      case 'security':
        return this.securityScan(targetPath, minSeverity);
      case 'complexity':
        return this.complexityAnalysis(targetPath);
      case 'review': {
      ...
      default:
        return `Error: unknown action "${action}". Use: security, complexity, review`;
    }
  }
  ...
  private securityScan(targetPath: string, minSeverity: string): string {
    ...
    const lines = filtered
      .slice(0, 50)
      .map((i) => `  [${icons[i.severity]}] ${i.file}:${i.line} ${i.rule} — ${i.message}`);
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (funcMatch) {
          ...
          if (currentFunc && i - funcStart > 50) {
            results.push(
              `  [!] ${filePath}:${funcStart + 1} "${currentFunc}" is ${i - funcStart} lines long (consider breaking up)`,
            );
          }
