...
export class TaskPlannerTool implements Tool {
  ...
  async execute(args: Record<string, unknown>): Promise<string> {
    ...
    switch (action) {
      ...
      case 'remove':
        return this.removeTask(args);
      case 'clear':
        return this.clearDone();
      default:
        return `Error: unknown action "${action}". Use: add, list, update, complete, remove, clear`;
    }
  }
  ...
  private addTask(args: Record<string, unknown>): string {
    ...
    const priority = (
      ['high', 'medium', 'low'].includes(args.priority as string) ? args.priority : 'medium'
    ) as Task['priority'];
  ...
  private listTasks(): string {
    ...
    const lines = tasks.map(
      (t) => `  ${icons[t.status] || '○'} #${t.id} [${priIcons[t.priority] || '!!'}] ${t.title} (${t.status})`,
    );
