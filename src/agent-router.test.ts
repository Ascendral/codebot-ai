 ...
 */
async function runOneTurn(agent: Agent, userMsg: string): Promise<void> {
...
describe('Agent — router OFF: behavior unchanged', () => {
  it('routerConfig undefined → model is never mutated', async () => {
    ...
    const { provider, modelsSeen } = makeModelCapturingProvider(() => (agentRef as unknown as { model: string }).model);
...
describe('Agent — router ON, no-op audit receipt (PR 11)', () => {
  it('emits router:no_op when desired tier maps to current model', async () => {
    let agentRef: Agent | null = null;
    const { provider } = makeModelCapturingProvider(() => (agentRef as unknown as { model: string }).model);
    const routerConfig: RouterConfig = {
      enabled: true,
      fastModel: 'claude-3-5-haiku-20241022',
    ...
    const routerRows = allLines.map((l) => JSON.parse(l)).filter((e) => e.tool === 'router');
    ...
    assert.strictEqual(routerRows[0].action, 'no_op', `expected action=no_op; saw ${routerRows[0].action}`);
