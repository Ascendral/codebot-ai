...
export function detectVRAM(): VRAMInfo {
  ...
  try {
    ...
    /* not NVIDIA or nvidia-smi not installed */
  }
...
export function recommendQuantization(model: string, vramMB: number): QuantRecommendation {
  ...
  const key = Object.keys(MODEL_SIZES).find(
    (k) => model.toLowerCase().includes(k.split(':')[0]) && model.includes(k.split(':')[1] || ''),
  );
  ...
  if (!params) {
    return {
      ...
      estimatedVRAM_MB: 0,
      fits: true,
      reason: 'Unknown model — defaulting to Q4_K_M (most compatible)',
    };
  }
  ...
  for (const level of levels) {
    ...
    if (est <= vramMB) {
      return {
        quantization: level,
        estimatedVRAM_MB: est,
        fits: true,
        reason: `${params}B params @ ${level} needs ~${est}MB, you have ${vramMB}MB`,
      };
    }
  }
  ...
  return {
    quantization: 'too_large',
    estimatedVRAM_MB: minEst,
    fits: false,
    reason: `${params}B params needs at least ~${minEst}MB even at Q4_K_M, you have ${vramMB}MB`,
  };
}
