...
describe('Solve — buildBranchName', () => {
  ...
  it('truncates long titles', () => {
    const branch = buildBranchName({
      number: 1,
      title: 'This is a very long issue title that should be truncated to keep branch name reasonable',
    });
...
describe('Solve — buildSolvePrompt', () => {
  const issue = {
    ...
    labels: ['bug'],
    comments: [{ user: 'alice', body: 'Can reproduce' }],
    url: 'https://github.com/test/app/issues/1',
    state: 'open',
  };
