...
const REASONING_TOOLS = new Set(['batch_edit', 'browser', 'ssh_remote', 'docker', 'code_review', 'routine']);
...
export function classifyComplexity(userMessage: string, lastToolCalls?: string[]): ModelTier {
  ...
  const reasoningPatterns = [
    ...
    /architect/i,
    /redesign/i,
    /security\s*(scan|audit|review)/i,
    /migrate/i,
    /optimize\s+(?:the|all|every)/i,
    /rewrite/i,
    /implement.*(?:system|feature|module)/i,
    /design\s+(?:a|the)/i,
    /fix.*(?:all|every|across)/i,
    /review.*(?:code|pr|pull)/i,
    /debug.*(?:complex|hard|tricky)/i,
    /multi.*file/i,
