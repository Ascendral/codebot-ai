...
const BUILTIN_WORKFLOWS: WorkflowDefinition[] = [
  {
    ...
    inputFields: [
      { name: 'topic', label: 'Topic or message', type: 'text', placeholder: 'What to post about...', required: true },
      {
        ...
        label: 'Tone',
        type: 'select',
        options: ['Professional', 'Casual', 'Witty', 'Informative'],
        default: 'Professional',
      },
    ],
    promptTemplate:
      'Compose and post a tweet on X about: {{topic}}. Use a {{tone}} tone. Keep it concise and engaging. Use the x_post tool to publish it.',
  },
  {
    ...
    inputFields: [
      {
        ...
        label: 'Thread topic',
        type: 'text',
        placeholder: 'What the thread is about...',
        required: true,
      },
      {
        name: 'points',
    ...
    promptTemplate:
      'Write a {{tweets}}-tweet thread for X about: {{topic}}. Key points to cover: {{points}}. Post the first tweet with x_post, then reply to it for the thread. Make it engaging and informative.',
  },
  {
    name: 'research-topic',
    ...
    inputFields: [
      { name: 'topic', label: 'Research topic', type: 'text', placeholder: 'What to research...', required: true },
      {
        ...
        label: 'Depth',
        type: 'select',
        options: ['Quick overview', 'Detailed analysis', 'Comprehensive report'],
      ...
      },
      {
        ...
        label: 'Output format',
        type: 'select',
        options: ['Summary', 'Bullet points', 'Full report'],
        default: 'Summary',
      },
    ],
    promptTemplate:
      'Research the following topic: {{topic}}. Provide a {{depth}} in {{format}} format. Use web_search and web_fetch to find current, reliable sources. Include key findings, data points, and source URLs.',
  },
  {
    name: 'generate-image',
    ...
    inputFields: [
      ...
      },
      {
        ...
        label: 'Style',
        type: 'select',
        options: ['Photorealistic', 'Digital art', 'Illustration', 'Minimalist', 'Abstract'],
        default: 'Digital art',
      },
      {
        name: 'size',
    ...
    promptTemplate:
      'Generate an image using the openai_images tool: {{description}}. Style: {{style}}. Size: {{size}}.',
  },
  {
    name: 'git-summary',
    ...
    inputFields: [
      { name: 'days', label: 'Days to look back', type: 'number', default: '7' },
      {
        ...
        label: 'Detail level',
        type: 'select',
        options: ['Brief', 'Detailed', 'With diffs'],
        default: 'Brief',
      },
    ],
    promptTemplate:
      'Summarize git activity from the last {{days}} days. Detail level: {{detail}}. Use the execute tool to run git log and git diff commands. Include commit count, files changed, and key changes.',
  },
  {
    name: 'check-website',
    ...
    inputFields: [
      { name: 'url', label: 'Website URL', type: 'text', placeholder: 'https://example.com', required: true },
      {
        ...
        label: 'What to check',
        type: 'select',
        options: ['Status only', 'Content analysis', 'Full audit'],
        default: 'Content analysis',
      },
    ],
    promptTemplate:
      'Check the website at {{url}}. Analysis level: {{checks}}. Use web_fetch to load the page. Report: status code, load time, main content summary, and any issues found.',
  },
  {
    name: 'daily-digest',
    ...
    inputFields: [],
    promptTemplate:
      'Generate a daily digest. Check: 1) Recent git commits (last 24h), 2) System health (disk, memory via execute tool), 3) Any scheduled routines and their status. Present as a clean, readable summary.',
  },
  {
    name: 'write-content',
    ...
    inputFields: [
      {
        ...
        label: 'Content type',
        type: 'select',
        options: ['Blog post', 'Article', 'Documentation', 'Email draft', 'Social media copy'],
        default: 'Blog post',
      },
      { name: 'topic', label: 'Topic', type: 'text', placeholder: 'What to write about...', required: true },
      {
        ...
        label: 'Length',
        type: 'select',
        options: ['Short (200 words)', 'Medium (500 words)', 'Long (1000+ words)'],
        default: 'Medium (500 words)',
      },
      {
        name: 'audience',
    ...
    promptTemplate:
      'Write a {{type}} about: {{topic}}. Target length: {{length}}. Audience: {{audience}}. Make it engaging and well-structured with a clear introduction, body, and conclusion.',
  },
  {
    name: 'send-email',
    ...
    inputFields: [
      ...
      {
        name: 'message',
        label: 'Message or instructions',
        type: 'textarea',
        placeholder: 'Write the email body or describe what the email should say...',
        required: true,
      },
      {
        name: 'tone',
    ...
    promptTemplate:
      'Compose and send an email to {{to}} with subject "{{subject}}". The message or instructions: {{message}}. Use a {{tone}} tone. Use the app gmail.send_email tool to send it.',
  },
  {
    ...
    inputFields: [
      { name: 'title', label: 'Meeting title', type: 'text', placeholder: 'Team standup...', required: true },
      {
        ...
        label: 'Date and time',
        type: 'text',
        placeholder: 'Tomorrow at 2pm, March 15 at 10am...',
        required: true,
      },
      {
        name: 'duration',
      ...
      {
        name: 'attendees',
        label: 'Attendees (emails, comma-separated)',
        type: 'text',
    ...
    promptTemplate:
      'Schedule a calendar event titled "{{title}}" on {{date}} for {{duration}}. Attendees: {{attendees}}. Notes: {{notes}}. Use the app google_calendar.create_event tool.',
  },
  {
    name: 'deep-research',
    ...
    inputFields: [
      { name: 'topic', label: 'Research topic', type: 'text', placeholder: 'What to research...', required: true },
      {
        ...
        type: 'select',
        options: ['Quick (2-3 sources)', 'Standard (4-5 sources)', 'Deep (6-8 sources)'],
        default: 'Standard (4-5 sources)',
      },
      {
        name: 'focus',
        label: 'Focus area (optional)',
        type: 'text',
        placeholder: 'e.g., recent developments, market analysis...',
      },
    ],
    promptTemplate:
      'Use the deep_research tool to research: {{topic}}. Depth: {{depth}}. Focus: {{focus}}. Present the findings in a clear, organized format with sources.',
  },
  {
    name: 'notion-update',
    ...
    inputFields: [
      {
        ...
        label: 'Action',
        type: 'select',
        options: ['Create new page', 'Update existing page', 'Search pages'],
        default: 'Create new page',
      },
      {
        name: 'title',
        label: 'Page title or search query',
        type: 'text',
        placeholder: 'Meeting notes, Project plan...',
        required: true,
      },
      { name: 'content', label: 'Content', type: 'textarea', placeholder: 'Page content or what to add...' },
    ],
    promptTemplate:
      'In Notion, {{action}}: "{{title}}". Content: {{content}}. Use the app notion.search or notion.create_page or notion.update_page tool as appropriate.',
  },
  {
    name: 'organize-drive',
    ...
    inputFields: [
      {
        ...
        label: 'Action',
        type: 'select',
        options: ['Search files', 'List recent files', 'Read a document'],
        default: 'Search files',
      },
      {
        name: 'query',
        label: 'Search query or file description',
        type: 'text',
        placeholder: 'Budget spreadsheet, project proposal...',
        required: true,
      },
    ],
    promptTemplate:
      '{{action}} on Google Drive: {{query}}. Use the app google_drive.search_files or google_drive.list_files or google_drive.read_file tool as appropriate. Present results clearly.',
  },
];
